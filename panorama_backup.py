...
def findHomography(image_1_kp, image_2_kp, matches):
    ...
    # WRITE YOUR CODE HERE.
    #n_match = len(matches)
    #for iMat in range(n_match):
    #	image_1_points[iMat] =  image_1_kp[matches[iMat].queryIdx].pt
    # 	image_2_points[iMat] =  image_2_kp[matches[iMat].trainIdx].pt

    image_1_points = np.array([[image_1_kp[match.queryIdx].pt] for match in matches])
    image_2_points = np.array([[image_2_kp[match.trainIdx].pt] for match in matches])
...
def getBoundingCorners(corners_1, corners_2, homography):
    ...
    """
    # WRITE YOUR CODE HERE
    corners_1_squ       = np.squeeze(corners_1)
    corners_1_ext       = np.concatenate((corners_1_squ, np.ones((4, 1))), axis=1)
    corners_1_trans     = np.dot(homography, corners_1_ext.T).T
    #print corners_1_trans.shape
    corners_1_trans_squ = np.array([[i/k, j/k] for (i, j, k) in corners_1_trans])
    corners_2_squ       = np.squeeze(corners_2)
    corners_comb        = np.vstack((corners_1_trans_squ, corners_2_squ))
    min_xy              = np.array([np.amin(corners_comb[:,0]), np.amin(corners_comb[:,1])])
    max_xy              = np.array([np.amax(corners_comb[:,0]), np.amax(corners_comb[:,1])])
...
def warpCanvas(image, homography, min_xy, max_xy):
    ...
    # WRITE YOUR CODE HERE
    trans_matrix = np.array([[1, 0, -min_xy[0]], [0, 1, -min_xy[1]], [0, 0, 1]])
    homo_com     = np.dot(trans_matrix, homography)
    warped_image = cv2.warpPerspective(image, homo_com, canvas_size)
...
def blendImagePair(image_1, image_2, num_matches):
    ...
    **************************************************************************
    ...
    #img2 = np.zeros(output_image.shape)

    #min_xy = min_xy.astype(np.int)
    #img2[-min_xy[1]:-min_xy[1] + image_2.shape[0],
    ...
    output_image = output_image * (1 - mask) + img2 * mask    
